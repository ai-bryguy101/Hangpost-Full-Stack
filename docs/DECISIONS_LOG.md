...
## 2026-05-29 — PR B: outcome capture uses the boolean-flag table additively

`recommendation_outcomes` is a 1:1 table (PK `impression_id`) with one
column per action (`viewed_at` timestamp; `profile_opened` /
`friend_request_sent` / `blocked` / `hangout_rsvped` booleans), not an
action-enum row-per-event log. So `POST .../outcomes` upserts: `viewed`
sets `viewed_at = now()`, the rest flip their boolean, and
`on_conflict_do_update` makes a view → open → friend-request sequence
**accumulate onto one row** rather than overwrite. The endpoint is
ownership-checked (404 if the impression isn't the caller's) so labels
can't be forged against someone else's recommendations.

## 2026-05-29 — `/demo` "Dismiss" does not write a `blocked` label

The original plan mapped a "not interested" button to the `blocked`
outcome. Dropped: nothing reads `recommendation_outcomes.blocked` to
exclude users (that's the separate `user_blocks` safety table), so
writing `blocked` on a soft dismiss would be a dishonest training label
and a misleading column for anyone reading the schema. Instead the demo
captures a clean **positive** signal — `friend_request_sent` via an "Add
friend" button — and "Dismiss" only hides the card client-side (no DB
write). A real negative-signal column can come with the Phase 3 UI if we
want one.

## 2026-05-29 — `features_json` is a sibling column, not an extension of `breakdown_json`

The 2026-05-29 "feature snapshot" entry left the column-vs-extend choice
to the PR. Chose a separate nullable `recommendation_impressions.features_json`
(Alembic `0003`): `breakdown_json` is the ranker's *output*, `features_json`
is its *input* — keeping them apart is cleaner for the Phase 7 trainer and
avoids overloading one blob. Nullable so impressions logged before the
column existed stay valid.

## 2026-05-29 — Impression ids generated in Python; batch `surfaced_at` pinned

`GET /recommendations` now mints `uuid4()` for each impression in Python
and uses it as both the PK and the `impression_id` returned to the client
(needed to POST outcomes) — avoids relying on bulk-insert `RETURNING`
ordering. The whole batch shares one Python-set `surfaced_at` so the
offline evaluator can group impressions into a single "query" by
`(source_user_id, surfaced_at)` deterministically, independent of DB
clock/driver batching.

## 2026-05-29 — Eval math is a pure stdlib module; committed report is synthetic-labeled

NDCG/Recall/baseline logic lives in `hangpost_api/recommendations/evaluation.py`
(stdlib only) so it unit-tests in CI without a DB and is reusable by the
Phase 7 retrainer; `scripts/evaluate.py` is a thin DB driver. The repo
can't run the DB-backed script here (no PyPI/DB), and fabricating a
"real" report on a portfolio repo would be dishonest — so the committed
`docs/eval/SYNTHETIC-self-test.md` is generated by the real module on
**synthetic** inputs and clearly labeled as such. The first real report
is a documented Codespaces step (`python -m scripts.evaluate`).
